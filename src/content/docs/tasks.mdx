...
- [ ] Implement `remark-dataview` plugin in `BrainDB` monorepo
  - support array of MDAST nodes in `remark-code-hook`?
...
- Functional metdata:
  ...
  - see other [frontmatter fields](https://starlight.astro.build/reference/frontmatter/)
- sidebar: show "new" (of "fresh") badge based on git update time?
- Footnotes
